...
func (c *Client) ForecastLocation (coordinate model.Coordinate) ([]*model.Forecast, error) {
    url := c.baseUrl + "/forecast.json" + c.urlArgs
    url  = fmt.Sprintf(url, c.key, strconv.FormatFloat(coordinate.Latitude, 'f', -5, 64), strconv.FormatFloat(coordinate.Longitude, 'f', -5, 64), c.days, c.language)
